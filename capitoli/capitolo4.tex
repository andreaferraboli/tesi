...
\subsection{Infrastruttura AWS di \textit{Finanz}}
...
Il layer applicativo, che ospita il servizio 'Finanz\discretionary{-}{}{-}Backend', è orchestrato tramite il servizio gestito \allowbreak{}\textbf{AWS \allowbreak{}Elastic \allowbreak{}Beanstalk}.
Tale servizio astrae la gestione dell'infrastruttura sottostante, automatizzando il provisioning e la configurazione delle risorse.
La configurazione prevede l'impiego di istanze EC2 di tipo \textbf{burstable} (`t3a.small`), definite tramite un Launch Template.
La scalabilità orizzontale è gestita da un Auto Scaling Group, le cui politiche sono basate su metriche di carico della CPU e sul numero di richieste al secondo, con soglie differenziate tra l'ambiente di sviluppo (da 1 a 2 istanze) e quello di produzione (da 3 a 5 istanze).
Un Application Load Balancer (ALB) funge da unico entry-point per il traffico HTTPS, instradandolo verso le istanze applicative.
Queste ultime sono protette da un Security Group che ne permette l'accesso esclusivamente dall'ALB, isolandole da accessi diretti.
Infine, i log applicativi e di sistema vengono inoltrati in tempo reale verso Amazon CloudWatch Logs per il monitoraggio e archiviati su Amazon S3 con rotazione oraria per l'analisi a lungo termine.
...
\textbf{Nota sui Placeholder:} Tutti gli identificatori di risorse AWS menzionati in questa sezione (subnet ID come \texttt{subnet\discretionary{-}{}{-}0a1b2c3d}, security group ID come \texttt{sg\discretionary{-}{}{-}0a1b2c3d4e5f67890}, instance ID come \texttt{i\discretionary{-}{}{-}0a1b2c3d4e5f67890}, NAT Gateway ID come \texttt{nat\discretionary{-}{}{-}0123456789abcdef0}, ecc.) sono placeholder utilizzati per motivi di privacy aziendale. In un ambiente reale, questi identificatori conterrebbero i valori specifici assegnati da AWS alle risorse dell'infrastruttura di produzione.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=1\textwidth]{aws_struttura}
  \caption{Diagramma semplificato dell’architettura AWS di \textit{Finanz}.}
  \label{fig:aws_struttura_attuale_cap2}
\end{figure}


\subsection{Modello Zero Trust e Principio del Minimo Privilegio}
...
\end{itemize}
\subsection{IAM come Pilastro di Zero Trust in AWS}
...
\begin{itemize}
    ...
    \item \textbf{Protezione Robusta delle Credenziali:} La password deve essere estremamente complessa e, soprattutto, l'\textbf{Auten\allowbreak{}ticazione a Più Fattori\allowbreak{}(MFA)} deve essere \textit{sempre} abilitata e richiesta per l'accesso root.
...
\subsection{Analisi IAM corrente di \emph{Finanz}}
...
Si è riscontrato che, nei 90 giorni precedenti l'analisi, l'account \emph{root} è stato utilizzato 76 volte per eseguire attività che potevano essere delegate a ruoli con privilegi limitati.
Tale pratica non è conforme al principio del minimo privilegio, che ne riserva l'uso a scenari di emergenza.
...
\begin{enumerate}
    ...
    \item \textbf{Autenticazione debole per gli utenti IAM}: Per gli account amministrativi non è obbligatorio l'uso della MFA per accedere alla console. Questo espone gli account al rischio di compromissione tramite furto di credenziali e viola la raccomandazione \textbf{IAM-5}.
...
\end{enumerate}
\section{Implementazione delle Migliorie IAM}
...
\begin{lstlisting}[style=json, caption={Policy IAM per negare eliminazioni in produzione}, label=lst:deny-prod-delete]
  {
    ...
    "Statement": [
        ...
        },
        {
            ...
            "Resource": [
                "arn:aws:iam::123456789012:role/CTO-AdminRole",
                "arn:aws:iam::123456789012:role/incident-responder",
  ...
  "Statement": [
    ...
    },
    {
       ...
       "Action": [
        ...
        "Condition": {
           "StringNotLike": {
              "iam:PermissionsBoundary": "arn:aws:iam::497487485332:policy/FinanzDeveloperBoundary" 
           }
        }
    }
  ]
...
\textbf{Nota:} L'account ID `497487485332` utilizzato nell'ARN della Permission Boundary è un placeholder utilizzato per motivi di privacy aziendale.

\subsection{Modello Ibrido per la Gestione degli Accessi}
...
\begin{description}
  \item[EC2] \texttt{dev}: permesso di \texttt{Start/Stop/Terminate} per le istanze di sviluppo.\newline \texttt{backend-dev}: stessi permessi del gruppo \texttt{dev}, con l'aggiunta di \texttt{Describe\discretionary{-}{}{}Images}.\newline \texttt{admin}: controllo completo, ad eccezione di azioni critiche come \texttt{DeleteVpc} in produzione (limitate tramite boundary o SCP).
  \item[Elastic Beanstalk] \texttt{dev}: autorizzazione al deploy negli ambienti di sviluppo.\newline \texttt{backend-dev}: deploy e salvataggio delle configurazioni (\texttt{eb config save}) negli ambienti di sviluppo.\newline \texttt{admin}: gestione dei template e delle versioni delle applicazioni, anche in produzione, previa adozione di specifiche cautele \cite{AWSEBRole}.
  \item[S3] \texttt{dev}: lettura/scrittura sui bucket con suffisso \texttt{-dev}.\newline \texttt{backend-dev}: dispone inoltre dei permessi \texttt{PutObjectAcl} su bucket di log specifici.\newline \texttt{admin}: facoltà di modificare policy di bucket (\texttt{PutBucketPolicy}) e configurazioni di replica.
  \item[RDS] \texttt{dev}: connessione in sola lettura (\texttt{rds-db:connect}) ai database di sviluppo.\newline \texttt{backend-dev}: esecuzione di statement tramite Data API sugli ambienti di sviluppo.\newline \texttt{admin}: creazione di snapshot (\texttt{CreateDBSnapshot}), avvio di task di esportazione (\texttt{StartExportTask}) e gestione del failover (\texttt{FailoverDBCluster}) \cite{AWSRDSIAM}.
\end{description}
...
\begin{itemize}
  ...
  \end{lstlisting}
    ...
    \item \textbf{Restrizione Geografica delle Regioni AWS}: Per ragioni di conformità normativa (ad esempio, il GDPR) e per ridurre la superficie d'attacco, si raccomanda di limitare l'operatività alle sole regioni AWS approvate (come \texttt{eu-central\discretionary{-}{}{-}1}, \texttt{eu-south\discretionary{-}{}{-}1} e \texttt{eu-west\discretionary{-}{}{-}1}). Una SCP può essere impiegata per impedire il provisioning di risorse in regioni non autorizzate \cite{awsbuilders:scps}.
    ...
    \begin{lstlisting}[style=json, caption={SCP per limitare le regioni AWS utilizzabili}, label=lst:scp-region-restriction]
{
  ...
  "Statement": [
    ...
    L'utilizzo della clausola \texttt{NotAction} è cruciale per escludere i servizi globali (come IAM e Route 53) che non sono legati a una regione specifica. La regione \texttt{us-east\discretionary{-}{}{-}1} viene inclusa in via eccezionale, poiché ospita gli endpoint di alcuni di questi servizi.
...
Viene creato un nuovo account AWS all'interno dell'Organization (\texttt{o\discretionary{-}{}{-}4g2j3d5e6l}), posizionato nella OU «Security». Questo account, con ID riservato \texttt{497487485332}, è mantenuto operativamente isolato, senza risorse operative e con un ciclo di fatturazione separato per garantirne l'integrità.

\item \textbf{Utente di emergenza \texttt{BreakGlassEmergencyUser}}:
All'interno dell'account Break-Glass, viene configurato un singolo utente IAM, il cui ARN è \texttt{arn:aws:iam::497487485332:user/BreakGlassEmergencyUser}. La sua sicurezza è affidata a una password di 32 caratteri generata casualmente, custodita in una busta fisica sigillata, e a un dispositivo MFA hardware FIDO2 dedicato (es. YubiKey, Seriale: \texttt{YK\discretionary{-}{}{-}87654321}). Per ridurre la superficie d'attacco, l'utente non dispone di access key permanenti e la sua unica abilità è quella di poter assumere il ruolo \texttt{BreakGlassAdminRole}.
...
\item \textbf{Monitoraggio e lockdown \allowbreak{}automatico}:
È implementato un sistema di monitoraggio intensivo.
Una regola EventBridge rileva ogni evento di login (\lstinline|aws.signin|) e invia una notifica immediata al topic SNS \texttt{security\discretionary{-}{}{-}alerts\discretionary{-}{}{-}breakglass}.
Inoltre, una funzione Lambda (\texttt{breakglass-auto-restrict}) viene attivata per applicare automaticamente una \emph{permissions-boundary} restrittiva al ruolo qualora la sessione superi la durata di 8 ore, limitando così la finestra di esposizione al rischio.
...
ROLE_ARN = os.environ["ROLE_TO_ASSUME"] # arn:aws:iam::497487485332:role/BreakGlassAdminRole
...
aws sts assume-role
--role-arn arn:aws:iam::497487485332:role/S3ReadOnlyForFinanzScript
...
\subsection{Implementazione di un Workflow di Approvazione a Due Fasi}
